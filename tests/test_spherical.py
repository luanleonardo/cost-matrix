...
def test_spherical_matrix():

    sources = np.array([[-23.456, -46.456]])
    destinations = np.array([[-23.456, -46.456], [-23.789, -46.789]])

    distance_matrix = cost_matrix.spherical(sources, destinations)

    assert distance_matrix.shape == (1, 2)


def test_spherical_matrix_km():
    """
    ...
    distance_matrix_meters = cost_matrix.spherical(
